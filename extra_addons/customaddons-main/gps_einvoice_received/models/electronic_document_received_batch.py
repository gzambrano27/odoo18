# -*- coding: utf-8 -*-
# -*- encoding: utf-8 -*-
# ⚙️ Actualizado para Odoo 18.0 — Warning eliminado del core.
...
# ==============================
# IMPORTS DE TOOLS PERSONALIZADOS
# ==============================
...
class ElectronicDocumentReceivedBatch(models.Model):
	...
	# --------------------------------------------------------
	# CREACIÓN DE DOCUMENTOS INDIVIDUALES
	# --------------------------------------------------------
	@api.model
	...
	def process_text(self, ext, brw_each, document_ids):
		if ext not in ("txt",):
			raise ValidationError(_("Extensión no soportada para procesar el archivo"))
	...
	def action_recover(self):
		...
		for brw_each in self:
			if brw_each.state == "ended":
				raise UserError(_("Lote de documentos ya procesado"))
			...
			if brw_each.document_ids:
				...
				document_ids += [-1, -1]
				srch_document_ids = self.env["electronic.document.received"].search([
					('state', 'not in', ('updated', "annulled")),
					('id', 'in', tuple(document_ids))
				])
 ...
 where x.state!='annulled' and x.state_sri!='annulled' """
		...
		srch_document_ids = self.env["electronic.document.received"].sudo().search(
			[('id', 'in', tuple(list_document_ids))])
		if srch_document_ids:
