...
class TestProjectTaskCommon(TransactionCase):
    ...
    def setUp(self):
        super(TestProjectTaskCommon, self).setUp()

        # Create test users
        self.test_user1 = self._create_internal_user(1)
        self.test_partner1 = self.test_user1.partner_id
        self.test_user2 = self._create_internal_user(2)
        self.test_partner2 = self.test_user2.partner_id
        ...
        invalid_ids = [self.test_user2.id, self.test_user1.id]
        while True:
            resource = self._create_resource("Resource A")
            if resource.id not in invalid_ids:
                self.test_resourceA = resource
                break
            resource.unlink()
        while True:
            resource = self._create_resource("Resource B")
            if resource.id not in invalid_ids:
                self.test_resourceB = resource
                break
            resource.unlink()

        # create test employees
        # Verify that underlying resource ids are unique
        invalid_ids.append(self.test_resourceA.id)
        invalid_ids.append(self.test_resourceB.id)
        while True:
            employee = self._create_employee(1)
            if employee.resource_id.id not in invalid_ids:
                self.test_employee1 = employee
                break
            employee.unlink()
        while True:
            employee = self._create_employee(2)
            if employee.resource_id.id not in invalid_ids:
                self.test_employee2 = employee
                break
            employee.unlink()
...
class TestRegular(TestProjectTaskCommon):
    def test_unique_ids(self):
        ...
        This is not interesting as a test by itself, but we had bugs in the
        past where things worked only because the employee id and resource id
        were the same and so we assert that they are different so that any such
        problem will hopefully become apparent in subsequent tests failing
        """
...
class TestControllers(TestProjectTaskCommon, HttpCase):
    """Class for tests on controllers"""

    def test_dont_load_specific_stages(self):
        task1 = self.task_1
        done_stage = self.env.ref("project.project_stage_2")
        cancelled_stage = self.env.ref("project.project_stage_3")
        task2 = self.task_2
        # we add as excluded stages "cancelled"
        task2.stage_id = cancelled_stage.id
        task1.stage_id = done_stage.id
        ctrl = BryntumGantt()
        with MockRequest(self.env):
            # we expect all tasks visible.
            task1.project_id.bryntum_dont_load_domain = False
            data = {"project_ids": [task1.project_id.id]}
            res = ctrl.bryntum_gantt_load(data=json.dumps(data))
            rows = res.get("tasks", {}).get("rows", [])
            children = rows[0].get("children", []) if rows else []
            self.assertEqual(len(children), 2)
            task1.project_id.bryntum_dont_load_domain = "[('stage_id', '=', %s)]" % (
                done_stage.id
            )
            res = ctrl.bryntum_gantt_load(data=json.dumps(data))
            rows = res.get("tasks", {}).get("rows", [])
            ...
            task1.project_id.bryntum_dont_load_domain = (
                "['|', ('stage_id', '=', %s), ('stage_id', '=', %s)]"
                % (done_stage.id, cancelled_stage.id)
            )
            res = ctrl.bryntum_gantt_load(data=json.dumps(data))
            rows = res.get("tasks", {}).get("rows", [])
            children = rows[0].get("children", []) if rows else []
