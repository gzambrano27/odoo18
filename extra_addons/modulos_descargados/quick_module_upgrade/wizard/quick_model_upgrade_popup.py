...
class QuickModelUpgrade(models.TransientModel):
    ...
    type_of_operation = fields.Selection([('upgrade','Upgrade'),('install','Install')],default='upgrade')

    '''For install modules list '''
    model_ids = fields.Many2many('ir.module.module', 'apps_models_relation', 'install_wizard_id', 'apps_id')


    def upgrade_button(self):
        if self.type_of_operation == 'upgrade':
            ...
            for module_obj in self.model_ids:
